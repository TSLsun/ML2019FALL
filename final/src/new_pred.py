...
if __name__ == "__main__":
   # detect is gpu available.
    ...
    CLASSNAME = ["Horse", "Bed", "Clock","Apple", "Cat", "Plane", "TV", "Dog", "Dolphin", "Spider"]


    dirs = os.listdir(checkpoint_dir)
    models=[ i for i in dirs if ('_G.pt' in i and i[:-5]+'_C1.pt' in dirs and i[:-5]+'_C2.pt' in dirs)]

    for name in models:
        print('current: ', name, flush=True)

        G = torch.load(checkpoint_dir+'/'+name, map_location=DEVICE)
        C1 = torch.load(checkpoint_dir+'/'+name[:-5]+'_C1.pt', map_location=DEVICE)
        C2 = torch.load(checkpoint_dir+'/'+name[:-5]+'_C2.pt', map_location=DEVICE)
        ...
        predictions_4=torch.zeros(num_elements,10).float()
        predictions_5=torch.zeros(num_elements,10).float()
        ...
        with torch.no_grad():
            for batch_idx, img_t in enumerate(target_dataloader):
                ...
                predictions_4[start:end] = output_t1
                predictions_5[start:end] = output_t2
        ...
        df.to_csv('pred/'+name[:-5]+'_pred_C1.csv',index=False)
        ...
        df.to_csv('pred/'+name[:-5]+'_pred_C1_C2.csv',index=False)

        dict1={'id': np.arange(0, len(predictions_4))}
        for i in range(10):
            dict1[CLASSNAME[i]]=predictions_4[:,i]
        df = pd.DataFrame(dict1)
        df.to_csv('pred/'+name[:-5]+'_pred_C1_10.csv',index=False)

        dict2={'id': np.arange(0, len(predictions_5))}
        for i in range(10):
            dict2[CLASSNAME[i]]=predictions_5[:,i]
        df = pd.DataFrame(dict2)
        df.to_csv('pred/'+name[:-5]+'_pred_C2_10.csv',index=False)
